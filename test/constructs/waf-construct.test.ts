...
import { WafConstruct } from '../../src/constructs/waf-construct';
...
describe('WafConstruct', () => {
  ...
  describe('Custom Rate Limit', () => {
    test('uses custom rateLimit value when provided', () => {
      new WafConstruct(stack, 'Waf', { rateLimit: 5000 });
      const template = Template.fromStack(stack);
      template.hasResourceProperties('AWS::WAFv2::WebACL', {
        Rules: Match.arrayWith([
          Match.objectLike({
            Name: 'RateLimitRule',
            Statement: { RateBasedStatement: { Limit: 5000 } },
          }),
        ]),
      });
    });
    ...
    test('uses different custom rateLimit value', () => {
      new WafConstruct(stack, 'Waf', { rateLimit: 1000 });
      const template = Template.fromStack(stack);
      template.hasResourceProperties('AWS::WAFv2::WebACL', {
        Rules: Match.arrayWith([
          Match.objectLike({
